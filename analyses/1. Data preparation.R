...
# Database queries 
  ...
  ## Better way : Connection to Mike's Access database
      ...
      MDBPATH <- "data/raw-data/fds_230424.accdb"
...
# Data preparation
  ## Merging Mike's team and INat identifications
    notINat <- df0 %>% 
      filter(!grepl("-", INat))%>%
      mutate(taxon.name = gn_parse_tidy(Valid)$canonicalsimple)
    
    df <- df0 %>% 
    filter(grepl("-", INat)) %>%
    left_join(inat_orchamp, by = "INat", relationship = "many-to-many") %>%
      bind_rows(notINat) %>%
    mutate(name = ifelse(is.na(taxon.name) == T, Valid, taxon.name))
  ...
  ## Merging the taxonomic backbone to the observations
    df1 <- left_join(df, valid_names, by = c("taxon.name" = "initial")) %>%
      select(!c(canonic, Valid, taxon.name, scientificName)) %>%
