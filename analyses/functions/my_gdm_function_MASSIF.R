...
#La fonction
my_gdm_function_MASSIF<-function(ENV, COMM, PHYTO, Methode, Variables, ECHELLE){
  ...
  ENV%>%
    ...
    select(c(Variables,massif))%>%
    group_by(massif)%>%
    summarise(across(
      .cols = all_of(Variables), 
      .fns = mean,
      na.rm=T))%>%
    rename(echelle = massif)%>%
    na.omit->ENV
  
  
  PHYTO%>%
    rename(id_plot=codeplot)%>%
    inner_join(ECHELLE, by='id_plot')->PHYTO
  
  COMM%>%
    unite(id_plot, gradient, alti)%>%
    inner_join(ECHELLE, by='id_plot')->COMM
